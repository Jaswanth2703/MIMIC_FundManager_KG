...
def build_ppt():
    ...
    add_body_text(slide, [
        "Method:",
        "  • Tests whether a variable's predictive relationship with the target",
        "    remains invariant across different environments (regime × fund type × VIX)",
        "  • Variables that are invariant are considered genuinely causal",
        "  • Confidence score ∈ [0, 1]: higher = stronger causal evidence",
        "  • Strictest causal claim among the three methods",
        "",
        "Results:",
        "  • Environments: small_cap, mid_cap (Fund_Type × regime combinations)",
        "  • 7 shared variables across strata (out of 12 per stratum)",
        "  • 1 variable confirmed by all three methods: volume_ratio",
        "  • Confidence threshold used: 0.15",
        "",
        "KG Integration:",
        "  • Each significant ICP result → CAUSES edge in Neo4j",
        "  • Properties: confidence, in_intersection, plausible_sets",
        "  • Highest confidence edges represent the strongest causal claims",
        "",
        "Reference: Peters, J., Bühlmann, P., & Meinshausen, N. (2016). Causal inference",
        "by using invariant prediction. Journal of the Royal Statistical Society: Series B, 78(5), 947–1012.",
    ], font_size=12)
    add_footer(slide, prs)
    ...
    add_body_text(slide, [
        ...
        "",
        "Results:",
        "  • 27 significant causal effects estimated",
        "  • Effect sizes: mean |θ̂| = 0.0023, max = 0.012",
        "  • Each effect comes with confidence intervals for uncertainty quantification",
        "",
        "KG Integration:",
    ...
    add_image_slide(slide, prs,
        ...
        subtitle="volume_ratio confirmed by all three methods — strongest causal evidence",
    ...
    add_image_slide(slide, prs,
        ...
        subtitle="Cohen's κ measures agreement with fund manager (Cohen, 1960) — fill after Phase 2 run",
    ...
    add_body_text(slide, [
        ...
        "  • ICP environments (fund type × regime) are relatively few",
        "  • DML effect sizes are small (mean |θ̂| = 0.0023) — may not be",
        "    economically significant despite statistical significance",
        "  • Causal variable utilization: 24.1% (58 variables, 14 used) —",
        "    many variables not reaching causal significance",
        "",
        "Model Limitations:",
        "  • Walk-forward backtest uses transaction cost assumption (0.5%)",
        "  • Equal-weighted portfolio for KG models (no confidence weighting)",
    ...
    add_body_text(slide, [
        ...
        "",
        "  ✓  Enriched the KG with 133 causal edges from three complementary methods",
        "     (Granger, ICP, DML) — achieving 100% FDR significance",
        "",
        "  ✓  Developed KG-native mimicry models (CBR, Path Transformer, CI-HGT)",
        "     that operate directly on the graph structure",
        "",
        "  ✓  Proposed three novel evaluation metrics (CSCS = 0.549, SCSI = 0.500,",
        "     DMF = 0.600) for causally-informed KG assessment",
        "",
        "  ✓  The CI-HGT CausalGate mechanism is a novel contribution that modulates",
        "     GNN message passing using learned causal edge strengths",
        "",
        "  ✓  Walk-forward backtest with real returns validates financial performance",
    ], font_size=12.5)
    add_footer(slide, prs)
